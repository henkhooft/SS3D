...
namespace SS3D.Utils
{
    public static class MathUtility
    {
        ...
        public static Vector2[] FindTilesOnLine(Vector2 firstPoint, Vector2 secondPoint)
        {
            ...
            FillTilesOnLine(
                Mathf.RoundToInt(firstPoint.x),
                Mathf.RoundToInt(firstPoint.y),
                Mathf.RoundToInt(secondPoint.x),
                Mathf.RoundToInt(secondPoint.y),
                list);
            return list.ToArray();
        }

        /// <summary>
        /// Allocation-friendly Bresenham fill. Uses rounded grid coords (same as tile snap).
        /// </summary>
        public static void FillTilesOnLine(int x0, int y0, int x1, int y1, List<Vector2> into)
        {
            into.Clear();
            int x = x0;
            int y = y0;

...
- **Ctrl+S panned the camera / Ctrl+O opened Unity's Open Scene:** map session WASD pan ignores Ctrl/Cmd/Alt. Open-map is **Ctrl/Cmd+Shift+O** (Unity's default shortcut profile is read-only, so temporarily clearing File/Open Scene is not viable).

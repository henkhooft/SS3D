...
- **Drag path stutters when moving fast:** was Instantiate/Destroy + `RpcSendCanBuild` per hologram per tile step (GC + network hitch). Pool inactive ghosts, preview can-build locally via `TryPreviewTile`, and only refresh holograms whose tile changed.

...
> Status: partial
> Verified: 671460029 — 2026-07-29 (NetworkAnimator observer gate restored)
...
- `Assets/Scripts/SS3D/Systems/Furniture/AirLockOpener.cs` — padded proximity auto-open; click Open/Close (`AirLockDoorInteraction`); access-denied SFX + door-light blink; SyncVar→animator; `NetworkAnimator` gated on observers; power-gated; `IDynamicTileOccupant` notifies [atmospherics](atmospherics.md)
...
- **Door animator vs NetworkAnimator:** keep prefab `Animator.cullingMode = CullCompletely` — do **not** toggle `Animator.enabled` via AOI `OnObserversActive` (AOI ≫ frustum; `KeepAnimatorStateOnDisable: 0` can desync Open). FishNet `NetworkAnimator` still ticks every FixedUpdate regardless of CullCompletely — gate `_networkAnimator.enabled` on observers only (`RefreshNetworkAnimatorCulling`). Hit 2026-07-29 (deep profile after `67146002`).

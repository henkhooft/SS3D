...
> Status: partial
> Verified: a403d978f — 2026-07-29 (probes Off recipe; SSAO off; Frame Debugger AI tooling)
...
- **Decal Layers must stay enabled:** `SS3D_ForwardPlusRenderer` Decal feature `decalLayers: 1`. Floor/bullet projectors target `ReceiveWorldDecals` only; characters stay Default. Turning layers off makes Lit tiles look fine while masking breaks for the intended filter path — do not disable during lighting retunes.
- **Do not drop DepthNormals when disabling SSAO:** DBuffer decals + Decal Layers require the DepthNormals prepass (`_WRITE_RENDERING_LAYERS`). SSAO was an extra consumer; with SSAO off the ~opaque-sized DepthNormals pass remains. Removing it re-breaks floor blood painting through the player ([health](health.md)). Hit 2026-07-29 (Frame Debugger).
- **SSAO stays off:** `ScreenSpaceAmbientOcclusion` `m_Active: 0` on Forward+. Half-toon + scarce fixtures make AO nearly invisible ([design/rendering-lighting.md](../../design/rendering-lighting.md)); re-enabling buys little look for real GPU cost.
- **GPU instancing still limited after probes Off:** Frame Debugger on Metastation — after Content probe clear + GenericShadeless instancing, remaining breaks are mostly different adjacency/door meshes and intentional emissive MPBs; do not expect floor-like instance counts on airlocks. Hit 2026-07-29.

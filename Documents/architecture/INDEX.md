...
| Health | [health](systems/health.md) | partial | Phases 1–5b shipped; screen-effects wired from snapshot; Phase 0d strips/rewires `Human.prefab`; vitals HUD / examine-self Phase 6 remainder (machine-interface health scanner reads the same snapshot as a third-party observer, but isn't the self-readout and isn't placeable yet); interim collapse RPCs — see [body-presentation-authority](2026-07_body-presentation-authority.md) |
...
| Machine interface UI | [machine-interface](systems/machine-interface.md) | partial | Diegetic APC/SMES/atmos/vending; path catalog; Dual Kawase blur + dim; DOTween bring-up/dismiss; health scanner code-only (no rebuilt asset catalog, no prefab, no imported art) |

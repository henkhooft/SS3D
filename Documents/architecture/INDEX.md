...
| audio | [audio.md](../design/audio.md) — active | [audio-foundation](2026-07_audio-foundation.md) — in-progress (Phase 1 SFX occlusion + Phase 2 per-area ambience + Phase 3 personal heartbeat/breathing shipped; Phase 0/4-5 pending) | [audio](systems/audio.md) — partial (split from chat-audio-screens) |
...
| Entities | [entities](systems/entities.md) | partial | Humanoids, minds, spawning; body-state animation + combat stances + injured limp/severity/mirror; Harm intent → combat stance; shelved Misc/Probably Not clips; `Human.prefab` composition debt; body presentation via `Ragdoll`/`BodyPresentationState` ([body-presentation-authority](2026-07_body-presentation-authority.md) shipped) |
| Health | [health](systems/health.md) | partial | Phases 1–5b shipped; screen-effects wired from snapshot; Phase 0d strips/rewires `Human.prefab`; vitals HUD / examine-self Phase 6 remainder; collapse intent via `BodyPresentationIntent` → `Ragdoll` ([body-presentation-authority](2026-07_body-presentation-authority.md) shipped); limp/`InjuredLeg`/arm injury → body anim ([animation-polish](2026-07_animation-polish.md)); zone resolve exclude-self + AnatomyNode for melee |
| Combat | [combat](systems/combat.md) | partial | Phase 0–1 melee + Phase 3 ranged hitscan (M4, accuracy cone, shared LOS, mag/reload); disarm/armor/projectile deferred |
...
| Chat / screens | [chat-audio-screens](systems/chat-audio-screens.md) | partial | Local speech chips + T-compose; always-on chat UI Phase 0 purged (headless ChatSubSystem); camera; camera ownership planned ([camera-ownership](2026-07_camera-ownership.md)) |
| Audio | [audio](systems/audio.md) | partial | Networked SFX/music pool (`AudioSubSystem`); client-local occlusion (`AudioSourceOcclusion`, seventh `LineOfSight` consumer); per-area ambience crossfade (`AmbienceSubSystem`) off synced `AreaRecord.AmbienceTrackId`; personal heartbeat/breathing (`PersonalAudioSubSystem` + Health/Stamina mappers); Boombox/NoisyCollision; legacy `AmbienceHandler` superseded |
...
| [2026-07_audio-foundation](2026-07_audio-foundation.md) | in-progress (Phase 1 client-local SFX occlusion + Phase 2 per-area ambience + Phase 3 personal heartbeat/breathing shipped; Phase 0 mixer groups, Phase 4 alerts, Phase 5 music/settings pending) |
...
| [combat_implementation_plan.md](../plans/combat_implementation_plan.md) | Clean-slate combat: Phase 0–1 melee + Phase 3 ranged hitscan shipped; disarm/stamina/armor later |

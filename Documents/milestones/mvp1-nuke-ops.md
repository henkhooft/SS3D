...
- **Env→health + seal + alarms + fire response (M8)** — turf exposure → systemic health; environmental armor seal ([armor.md](../design/armor.md) §3); air alarms that **actually alarm** on bad air (controllers exist — [atmospherics](../architecture/systems/atmospherics.md)); extinguisher / thin fire counter; internals ride M0 loadouts. Equal-pressure composition diffusion on turf is shipped (breath pockets mix into the room — [atmospherics](../architecture/systems/atmospherics.md)); seal / alarms-that-alarm / fire response still open.
...
| M8 | Env→health + armor seal + internals + air alarms that alarm + thin fire response | pending — **focus**; client atmos VFX + equal-P turf diffusion shipped | [armor.md](../design/armor.md) §3; [health](../architecture/systems/health.md); [atmospherics](../architecture/systems/atmospherics.md); [area.md](../design/area.md) §5 |
